...
namespace ScrewTurn.Wiki {
	...
	public static class Formatter {
        ...
        private static readonly Regex GalleryRegex = new Regex(@"\<gallery\s*?(perrow=(?<perrow>\d*))?\s*?(size=(?<size>\d*))?\s*?\>(?<val>(?sm-x).*?)<\/gallery\>", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Multiline);
        ...
        private static readonly Regex JavascriptCommentRegex = new Regex(@"(?<!(\<script.*?\>[\s\n]*))\<\!\-\-.*?\-\-\>(?!([\s\n]*\<\/script\>))", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.CultureInvariant);
        private static readonly Regex StyleRegex = new Regex(@"\<style.*?\>.*?\<\/style\>", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.CultureInvariant);
        private static readonly Regex StyleCommentRegex = new Regex(@"(?<!(\<style.*?\>[\s\n]*))\<\!\-\-.*?\-\-\>(?!([\s\n]*\<\/style\>))", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.CultureInvariant);
		...
		/// <param name="currentPageFullName">The current page full name, if any.</param>
		/// <returns>The current namespace (<c>null</c> for the root).</returns>
		private static NamespaceInfo DetectNamespaceInfo(string wiki, string currentPageFullName) {
			if(currentPageFullName == null) {
				return Tools.DetectCurrentNamespaceInfo(wiki);
			}
			else {
				string ns = NameTools.GetNamespace(currentPageFullName);
		...
		/// <param name="currentPageFullName">The full name of the current Page (can be null).</param>
		/// <returns>The formatted text.</returns>
		public static string Format(string wiki, string raw, bool forIndexing, FormattingContext context, string currentPageFullName) {
			string[] tempLinks;
			return Format(wiki, raw, forIndexing, context, currentPageFullName, out tempLinks);
		}
		...
		/// <returns>The formatted text.</returns>
		public static string Format(string wiki, string raw, bool forIndexing, FormattingContext context, string currentPageFullName, out string[] linkedPages) {
			return Format(wiki, raw, forIndexing, context, currentPageFullName, out linkedPages, false);
		}
		...
		/// <returns>The formatted text.</returns>
		public static string Format(string wiki, string raw, bool forIndexing, FormattingContext context, string currentPageFullName, out string[] linkedPages, bool bareBones) {
...
*/
            ...
            // Hacks remove between html-tags
            sb.Replace(">\n<", "><"); // NOTE:1
            ...
            // Remove all double- or single-LF in Style tags
            match = StyleRegex.Match(sb.ToString());
			while(match.Success) {
				sb.Remove(match.Index, match.Length);
				if(singleLine) sb.Insert(match.Index, match.Value.Replace("\n", ""));
				else sb.Insert(match.Index, match.Value.Replace("\n\n", "\n"));
                match = StyleRegex.Match(sb.ToString(), match.Index + 1);
			}
			...
			if(!bareBones) {
				NamespaceInfo ns = DetectNamespaceInfo(wiki, currentPageFullName);
							...
							switch(match.Value.Substring(1, match.Value.Length - 2).ToUpperInvariant()) {
								...
								case "SEARCHBOX":
                                    ...
                                    sb.Insert(match.Index, doSearchFunction +
                                        @"<div id=""simpleSearch"">	<input class=""txtsearchbox"" type=""text"" id=""" + textBoxId + @""" onkeydown=""javascript:var keycode; if(window.event) keycode = event.keyCode; else keycode = event.which; if(keycode == 10 || keycode == 13) { _DoSearch_" + textBoxId + @"(); return false; }"" type=""search""/><input name=""go"" value="""" title="""" id=""searchButton"" class=""searchButton"" type=""submit"" onclick=""javascript:_DoSearch_" + textBoxId + @"(); return false;""></div></nowiki></nobr>");
                                    //sb.Insert(match.Index, doSearchFunction +
								...
								case "CATEGORIES":
									...
									break;
								case "CLOUD":
									string cloud = BuildCloud(wiki, DetectNamespaceInfo(wiki, currentPageFullName));
									sb.Insert(match.Index, cloud);
									break;
								case "PAGECOUNT":
									sb.Insert(match.Index, Pages.GetPages(wiki, DetectNamespaceInfo(wiki, currentPageFullName)).Count.ToString());
									break;
								case "PAGECOUNT(*)":
									sb.Insert(match.Index, Pages.GetGlobalPageCount(wiki).ToString());
									break;
								case "ORPHANS":
									sb.Insert(match.Index, BuildOrphanedPagesList(wiki, DetectNamespaceInfo(wiki, currentPageFullName), context, currentPageFullName));
									break;
								case "WANTED":
									sb.Insert(match.Index, BuildWantedPagesList(wiki, DetectNamespaceInfo(wiki, currentPageFullName)));
									break;
								case "NAMESPACELIST":
			...
			while(match.Success) {
				...
				a = "";
				n = "";
                if (tmp.ToLowerInvariant().StartsWith("image:")) // new image format
                {
                    done = true;
                    string f = tmp.Substring("image:".Length);
                    ...
                    for (int i = 1; i < fields.Length; i++)
                    {
                        ...
                        if (field.ToLowerInvariant().StartsWith("max"))
                        {
                            int number;
                            if (Int32.TryParse(field.Substring(3), out number))
                            {
                                sizeW = number;
                                isMax = true;
                                bbsize = "customMax";
                        ...
                        if (field.ToLowerInvariant().StartsWith("link="))
                        {
                            bigUrl = field.Substring(5);
                            isLink = true;
                            continue;
                        }

                        if (field.ToLowerInvariant().StartsWith("alt="))
                        {
                            alt = field.Substring(4);
                            continue;
                        }

                        Regex reg = new Regex(@"^\d{1,4}x\d{1,4}$");
                        var m = reg.Match(field);
                        if (!String.IsNullOrEmpty(m.Groups[0].Value))
                        {
                            var sz = m.Groups[0].Value.Split('x');
                            sizeW = Convert.ToInt32(sz[0]);
                            sizeH = Convert.ToInt32(sz[1]);
                            continue;
                        }
                        ...
                        {
                             int num;
                            if (Int32.TryParse(field, out num))
                            {
                                sizeW = num;
                                bbsize = "customWidth";
                                continue;
                    ...
                    if (sizeW > 0 && urlType == "wiki")
                        url = String.Format("{0}|{1}{2}", url, isMax ? "m" : "w", sizeW);
                    url = EscapeUrl(url);
                    ...
                    if (bareBones)
                    {
                        var parameters = new StringBuilder(100);
                        parameters.AppendFormat("wikiid=\"image\"");
                        parameters.AppendFormat(@"data-wiki-type-size=""{0}"" data-wiki-align=""{1}"" data-wiki-border=""{2}"" data-wiki-size-width=""{3}"" ", bbsize, bbalign, bbframe, sizeW);
                        parameters.AppendFormat(@"data-wiki-url-type=""{0}"" data-wiki-link=""{1}"" data-wiki-desc=""{2}""  ", urlType, isLink ? bigUrl : String.Empty, title);

                        var style = new StringBuilder(100);
                        style.Append("margin: 10px;");
                        if (sizeW > 0)
                            style.AppendFormat(@" width: {0}px;", sizeW);
                        if (sizeH > 0)
                            style.AppendFormat(@" height: {0}px;", sizeH);
                        if (bbalign != "auto")
                            style.AppendFormat(@" float: {0};", bbalign);

                        img.Append(@"<img src=""");
                        img.Append(url);
                        img.AppendFormat(@""" alt=""{0}""", alt);
                        //if (title.Length > 0)
                        //    img.Append(StripWikiMarkup(StripHtml(title.TrimStart('#'))));
                        //else
                        //    img.Append(Exchanger.ResourceExchanger.GetResource("Image"));
                        img.AppendFormat(@" style=""{0}"" {1}/>", style, parameters);
                        //img.Append(BuildLink(wiki, bigUrl, dummy.ToString(), true, title, forIndexing, bareBones,
                        //                     context, null, tempLinkedPages, sizeW == 0 ? null : "picture"));
                    }
                    else
                    {
                        ...
                        {
                            img.AppendFormat(@"<table {0} cellpadding=""0"" cellspacing=""0""><tr><td>", bbframe == "frame" ? @"class=""imageauto""" : "");
                        }
                        else
                        {
                            if (bbframe == "frame")
                            ...
                            {
                                img.Append(@"<div style=""");
                                img.AppendFormat("float: {0}", bbalign);      
                            }
                            img.Append(@""">");
                        }
                        var style = new StringBuilder(100);
                        ...
                        if (sizeH > 0)
                            style.AppendFormat(@" height: {0}px;", sizeH);
                        img.AppendFormat(@"<div class=""imagethumbinner""  style=""{0}"">", style);

                        dummy = new StringBuilder(200);
                        dummy.AppendFormat(@"<img class=""image"" style=""{0}{1}"" src=""", style, bbframe == "frameless" ? @"border: none;" : "");
                        dummy.Append(url);
                        dummy.AppendFormat(@""" alt=""{0}""", alt);
                        //if (title.Length > 0)
                        ...
                        {
                            img.Append(@"<div class=""imagedescription"">");
                            if (sizeW != 0)
                            {
                                img.Append(@"<div class=""magnify"">");
                                img.Append(@"<a href=""");
                                img.Append(bigUrl);
                ...
                else if (tmp.ToLowerInvariant().StartsWith("file:")) // new file format
                {
                    done = true;
                    string f = tmp.Substring("file:".Length);
                    string[] fields = f.Split('|');
                    url = EscapeUrl(fields[0]);
                    title = fields.Length > 1 ? fields[1] : fields[0];
                    var filelink = new StringBuilder();
                    if (bareBones)
                    {
                        //if (url.StartsWith())
                        filelink.AppendFormat(@"<a class=""internallink"" wikiid=""file"" href=""{0}""", url);
                        filelink.AppendFormat(@" title=""{0}"">{0}</a>", title);
                    }
                    else
                    {
                    ...
                    sb.Insert(match.Index, filelink);
                }
                else if (tmp.IndexOf("|") != -1) // obsolete image format
                {
                    ...
                    else
                    {
                        ...
                        if (fields[0].ToLowerInvariant().Equals("imageleft") || fields[0].ToLowerInvariant().Equals("imageright") || fields[0].ToLowerInvariant().Equals("imageauto"))
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    img.Append(
                                        @"<table class=""imageauto"" cellpadding=""0"" cellspacing=""0""><tr><td>");
                                }
                                else
                                {
                                    img.Append(@"<div class=""");
                    ...
                    // This is an attachment string[] fields = tmp.Split('|');
					done = true;
            ...
            //{
                int galleryIndex = 1;
                match = GalleryRegex.Match(sb.ToString());
                while (match.Success)
                {
                    if (!IsNoWikied(match.Index, noWikiBegin, noWikiEnd, out end))
                    {
                        ...
                        int number;
                        var size = 0;
                        if (Int32.TryParse(match.Groups["size"].Value, out number))
                            size = number;
                        var perrow = 0;
                        if (Int32.TryParse(match.Groups["perrow"].Value, out number))
                        {
                            perrow = number;
                        }
                        //else
                        ...
                        //    if (size == 0)
                        //    {
                        //        perrow = 1; // default count picture in line (size = 0? that full size!)
                        //    }
                        //    else
                        //    {
                        //        perrow = 1000/size;
                        ...
                        string imageRel = size == 0 ? null : String.Format("prettyPhoto[pp_gal{0}]", galleryIndex);

                        List<string> imgl = new List<string>();
                        Regex pattern = new Regex(@"image:(?<val>.*?)(?:\n)", RegexOptions.Compiled | RegexOptions.Singleline);
                        foreach (Match m in pattern.Matches(match.Groups["val"].Value))
                            if (m.Success)
                                imgl.Add(m.Groups["val"].Value);
                        ...
                        if (imgl.Count > 0)
                        {
                            ...
                            if (bareBones)
                            {
                                img.Append("<div wikiid=\"gallery\" meta=\"");
                                if (perrow > 0)
                                    img.AppendFormat("perrow={0};", perrow);
                                if (size > 0)
                                    img.AppendFormat("size={0};", size);
                                img.Append("\">");

                                foreach (var i in imgl)
                                {
                                    ...
                                    url = fields[0].Trim();
                                    if (size > 0)
                                        url = String.Format("{0}|m{1}", url, size);

                                    url = EscapeUrl(url);
                                    img.Append(@"<img src=""");
                                    img.Append(url);
                                    img.Append(@""" style=""margin: 10px;"" alt=""");

                                    title = fields.Length > 1 && !String.IsNullOrWhiteSpace(fields[1]) ? fields[1].Trim() : " ";
                                    if (title.Length > 1)
                                        img.Append(StripWikiMarkup(StripHtml(fields[1].Trim().TrimStart('#'))));
                            ...
                            else
                            {
                                ...
                                img.Append(String.Format(
                                    "<ul class=\"gallery\" style=\"max-width: {0}px;_width: {0}px;\">",
                                    perrow*(size + 28)));
                                foreach (var i in imgl)
                                {
                                    img.Append(String.Format("<li class=\"gallerybox\" style=\"width: {0}px\">",
                                                             size + 25));
                                    img.Append(String.Format("<div style=\"width: {0}px\">", size + 25));
                                    img.Append(String.Format("<div class=\"thumb\" style=\"width: {0}px;\">", size + 20));
                                    img.Append("<div style=\"margin:10px auto;\">");
                                    ...
                                    dummy.Append(url);
                                    dummy.Append(@""" alt=""");
                                    title = fields.Length > 1 && !String.IsNullOrWhiteSpace(fields[1])
                                                ? fields[1].Trim()
                                                : " ";
                                    // white space else url will in title
                                    if (title.Length > 1)
                                        dummy.Append(StripWikiMarkup(StripHtml(fields[1].Trim().TrimStart('#'))));
                                    //else
                                    //    dummy.Append(Exchanger.ResourceExchanger.GetResource("Image"));
                                    dummy.Append(@""" />");
                                    img.Append(BuildLink(wiki, currentNamespace, bigUrl, dummy.ToString(), true, title, forIndexing,
                                    ...
                                    img.Append(
                                        String.Format("</div></div><div class=\"gallerytext\">{0}</div></div></li>",
                                                      title));
                                }
                    ...
                    ComputeNoWiki(sb.ToString(), ref noWikiBegin, ref noWikiEnd);
                    match = GalleryRegex.Match(sb.ToString(), end);
                }
            //}
			...
			if(!bareBones && currentPageFullName != null) {
				...
				while(match.Success) {
					if(!IsNoWikied(match.Index, noWikiBegin, noWikiEnd, out end)) {
						...
						if(balanced.IndexOf("}") == balanced.Length - 1) {
							...
							string[] temp = null;
							sb.Insert(match.Index,
								Format(wiki, FormatSnippet(wiki, balanced, tocString), forIndexing, context, currentPageFullName, out temp, bareBones).Trim('\n'));
							...
							do {
								...
								string[] temp;
								formattedInternalSnippet = Format(wiki, formattedInternalSnippet, forIndexing, context, currentPageFullName, out temp, bareBones).Trim('\n');
								if(temp != null) tempLinkedPages.AddRange(temp);
			...
			if(!bareBones) {
                match = JavascriptCommentRegex.Match(sb.ToString());
				while(match.Success) {
					sb.Remove(match.Index, match.Length);
					sb.Insert(match.Index, " "); // This prevents the creation of additional blank lines
                    match = JavascriptCommentRegex.Match(sb.ToString(), match.Index + 1);
				}
			}

            // Style out all comments
			if(!bareBones) {
                match = StyleCommentRegex.Match(sb.ToString());
				while(match.Success) {
					sb.Remove(match.Index, match.Length);
					sb.Insert(match.Index, " "); // This prevents the creation of additional blank lines
                    match = StyleCommentRegex.Match(sb.ToString(), match.Index + 1);
				}
			}
            ...
            ProcessLineBreaks(wiki, sb, bareBones, noWikiBegin, noWikiEnd);
		...
		/// <param name="context">The formatting context.</param>
		/// <param name="currentPageFullName">The current page full name, or <c>null</c>.</param>
		/// <returns>The recent changes list HTML markup.</returns>
		private static string BuildRecentChanges(string wiki, NamespaceInfo currentNamespace, bool allNamespaces, FormattingContext context, string currentPageFullName) {
			...
			return BuildRecentChangesTable(wiki, allChanges, context, currentPageFullName);
		}
		...
		public static string BuildRecentChangesTable(string wiki, IList<RecentChange> allChanges, FormattingContext context, string currentPageFullName) {
			...
			for(int i = 0; i < maxChanges; i++) {
				...
				sb.Append("</td><td>");
				sb.Append(PrintRecentChange(wiki, allChanges[i], context, currentPageFullName));
				sb.Append("</td>");
				sb.Append("</tr>");
			}
		...
		private static string PrintRecentChange(string wiki, RecentChange change, FormattingContext context, string currentPageFullName) {
			switch(change.Change) {
				case Change.PageUpdated:
					return Exchanger.ResourceExchanger.GetResource("UserUpdatedPage").Replace("##USER##", Users.UserLink(wiki, change.User)).Replace("##PAGE##", PrintPageLink(wiki, change, context, currentPageFullName));
				case Change.PageRenamed:
					return Exchanger.ResourceExchanger.GetResource("UserRenamedPage").Replace("##USER##", Users.UserLink(wiki, change.User)).Replace("##PAGE##", PrintPageLink(wiki, change, context, currentPageFullName));
				case Change.PageRolledBack:
					return Exchanger.ResourceExchanger.GetResource("UserRolledBackPage").Replace("##USER##", Users.UserLink(wiki, change.User)).Replace("##PAGE##", PrintPageLink(wiki, change, context, currentPageFullName));
				case Change.PageDeleted:
					return Exchanger.ResourceExchanger.GetResource("UserDeletedPage").Replace("##USER##", Users.UserLink(wiki, change.User)).Replace("##PAGE##", PrintPageLink(wiki, change, context, currentPageFullName));
				case Change.MessagePosted:
					return Exchanger.ResourceExchanger.GetResource("UserPostedMessage").Replace("##USER##", Users.UserLink(wiki, change.User)).Replace("##PAGE##", PrintMessageLink(wiki, change, context, currentPageFullName));
				case Change.MessageEdited:
					return Exchanger.ResourceExchanger.GetResource("UserEditedMessage").Replace("##USER##", Users.UserLink(wiki, change.User)).Replace("##PAGE##", PrintMessageLink(wiki, change, context, currentPageFullName));
				case Change.MessageDeleted:
					return Exchanger.ResourceExchanger.GetResource("UserDeletedMessage").Replace("##USER##", Users.UserLink(wiki, change.User)).Replace("##PAGE##", PrintMessageLink(wiki, change, context, currentPageFullName));
				default:
		...
		private static string PrintPageLink(string wiki, RecentChange change, FormattingContext context, string currentPageFullName) {
			PageContent page = Pages.FindPage(wiki, change.Page);
			if(page != null) {
				return string.Format(@"<a href=""{0}{1}"" class=""pagelink"">{2}</a>",
					change.Page, GlobalSettings.PageExtension, FormattingPipeline.PrepareTitle(wiki, change.Title, false, context, currentPageFullName));
			}
			else {
				return FormattingPipeline.PrepareTitle(wiki, change.Title, false, context, currentPageFullName) + " (" + change.Page + ")";
			}
		}
		...
		/// <param name="currentPageFullName">The current page full name, or <c>null</c>.</param>
		/// <returns>The link HTML markup.</returns>
		private static string PrintMessageLink(string wiki, RecentChange change, FormattingContext context, string currentPageFullName) {
			PageContent page = Pages.FindPage(wiki, change.Page);
			if(page != null) {
				return string.Format(@"<a href=""{0}{1}?Discuss=1#{2}"" class=""pagelink"">{3}</a>",
					...
					FormattingPipeline.PrepareTitle(wiki, change.Title, false, context, currentPageFullName) + " (" +
					FormattingPipeline.PrepareTitle(wiki, change.MessageSubject, false, context, currentPageFullName) + ")");
			}
			else {
				return FormattingPipeline.PrepareTitle(wiki, change.Title, false, context, currentPageFullName) + " (" + change.Page + ")";
			}
		...
		private static string BuildOrphanedPagesList(string wiki, NamespaceInfo nspace, FormattingContext context, string currentPageFullName) {
			string[] orhpans = Pages.GetOrphanedPages(wiki, nspace);
			...
			foreach(string page in orhpans) {
				PageContent content = Pages.FindPage(wiki, page);
				sb.Append("<li>");
				sb.AppendFormat(@"<a href=""{0}{1}"" title=""{2}"" class=""pagelink"">{2}</a>", Tools.UrlEncode(page), GlobalSettings.PageExtension,
					FormattingPipeline.PrepareTitle(wiki, content.Title, false, context, currentPageFullName));
				sb.Append("</li>");
			}
		...
		/// <param name="currentPageFullName">The current page full name, if any.</param>
		/// <returns>The list.</returns>
		private static string BuildIncomingLinksList(string wiki, string pageFullName, FormattingContext context, string currentPageFullName) {
			if(pageFullName == null) return "";

			string[] links = Pages.GetPageIncomingLinks(wiki, pageFullName);
			...
			foreach(string link in links) {
				PageContent linkedPage = Pages.FindPage(wiki, link);
				if(linkedPage != null) {
					sb.Append("<li>");
					sb.AppendFormat(@"<a href=""{0}{1}"" title=""{2}"" class=""pagelink"">{2}</a>", Tools.UrlEncode(link), GlobalSettings.PageExtension,
						FormattingPipeline.PrepareTitle(wiki, linkedPage.Title, false, context, currentPageFullName));
					sb.Append("</li>");
				}
			}
		...
		/// <param name="currentPageFullName">The current page full name, if any.</param>
		/// <returns>The list.</returns>
		private static string BuildOutgoingLinksList(string wiki, string pageFullName, FormattingContext context, string currentPageFullName) {
			if(pageFullName == null) return "";

			string[] links = Pages.GetPageOutgoingLinks(wiki, pageFullName);
			if(links.Length == 0) return "";
		...
		private static string BuildNamespaceList(string wiki) {
			...
			sb.Append("<ul>");
			sb.Append("<li>");
            sb.Append(BuildNamespaceLink(wiki, null));
			sb.Append("</li>");
			foreach(NamespaceInfo ns in Pages.GetNamespaces(wiki)) {
				sb.Append("<li>");
                sb.Append(BuildNamespaceLink(wiki, ns.Name));
				sb.Append("</li>");
			}
	    ...
	    /// </summary>
	    /// <param name="wiki">The wiki.</param>
	    /// <param name="sb">The <see><cref>T:StringBuilder</cref></see> containing the text to process.</param>
	    /// <param name="bareBones">A value indicating whether the formatting is being done in bare-bones mode.</param>
	    /// <param name="noWikiBegin"></param>
	    /// <param name="noWikiEnd"></param>
	    private static void ProcessLineBreaks(string wiki, StringBuilder sb, bool bareBones, List<int> noWikiBegin, List<int> noWikiEnd)
        {
			if(AreSingleLineBreaksToBeProcessed(wiki)) {
				...
				//sb.Replace("\n", "<br />");
                // Replace all \n to <br />, but except in tag <nowiki>
                int end;
                Regex r = new Regex(@"\n", RegexOptions.Compiled | RegexOptions.Singleline);
                match = r.Match(sb.ToString());
                while (match.Success)
                {
                    if (!IsNoWikied(match.Index, noWikiBegin, noWikiEnd, out end))
                    {
            ...
            bool forIndexing, bool bareBones, FormattingContext context, string currentPageFullName, List<string> linkedPages, string rel = null)
        {
			...
			if(targetUrl.ToLowerInvariant().Equals("anchor") && title.StartsWith("#")) {
				...
				// Link to an internal file or subdirectory, or link to GetFile
				sb.Append(@"<a");
				if(!isImage) sb.Append(@" class=""internallink""");
				if(blank) sb.Append(@" target=""_blank""");
                // mc
				...
				if(targetUrl.IndexOf(".aspx") != -1) {
					...
					if(targetUrl.StartsWith("c:") || targetUrl.StartsWith("C:")) {
						...
						if(!explicitNamespace) {
							fullName = currentPageFullName != null ?
								 NameTools.GetFullName(NameTools.GetNamespace(currentPageFullName), NameTools.GetLocalName(tempLink)) :
								 tempLink;
						}
						...
						PageContent pageContent = Pages.FindPage(wiki, fullName);
						if(pageContent != null) {
							if(!linkedPages.Contains(pageContent.FullName)) linkedPages.Add(pageContent.FullName);
						}
						else {
						...
						if(pageContent == null) {
                            ...
                            if (ConfigurationManager.AppSettings["ScrewTurnWiki.CreateInternalLinks"] == "1")
                            {
                                sb.Append(@"<a");
                            ...
                            {
                                sb.Append("<a class=\"pagenotfound\">" + fullName + "</a>");
                            }
						}
						else {
							sb.Append(@"<a");
							...
							if(forIndexing) {
								...
								if(currentPageFullName != null && currentPageFullName == pageContent.FullName) {
									// Do not format title
									sb.Append(pageContent.FullName);
								}
								else {
									// Try to format title
									if(pageContent != null && !pageContent.IsEmpty()) {
										sb.Append(FormattingPipeline.PrepareTitle(wiki, pageContent.Title, forIndexing, context, currentPageFullName));
									}
									else sb.Append(pageContent.FullName);
								}
							}
							else sb.Append(FormattingPipeline.PrepareTitle(wiki, pageContent.Title, forIndexing, context, currentPageFullName));
        ...
        /// <returns></returns>
        private static string[] SplitTable(string table)
	    {
		...
		public static string FormatPhase3(string wiki, string raw, FormattingContext context, string currentPageFullName) {
			...
			while(match.Success) {
				...
				switch(match.Value.Substring(1, match.Value.Length - 2).ToUpperInvariant()) {
					...
					case "INCOMING":
						...
						break;
					case "OUTGOING":
						sb.Insert(match.Index, BuildOutgoingLinksList(wiki, currentPageFullName, context, currentPageFullName));
						break;
					case "USERNAME":
						...
						break;
					case "PAGENAME":
						if(currentPageFullName != null) sb.Insert(match.Index, currentPageFullName);
						break;
					case "LOGINLOGOUT":
			...
			if(currentPageFullName != null) {
				...
				while(match.Success) {
					...
					NamespaceInfo currentNamespace = currentNamespace = Pages.FindNamespace(wiki, NameTools.GetNamespace(currentPageFullName));
					sb.Insert(match.Index, BuildRecentChanges(wiki, currentNamespace, trimmedTag.EndsWith("(*)"), context, currentPageFullName));
					match = RecentChangesRegex.Match(sb.ToString());
				}
			}
			...
			if(currentPageFullName != null && context == FormattingContext.PageContent) {
				bool canEdit = false;
				bool canEditWithApproval = false;

				Pages.CanEditPage(wiki, currentPageFullName, SessionFacade.GetCurrentUsername(), SessionFacade.GetCurrentGroupNames(wiki),
			...
			while(match.Success) {
				...
				if(pageName.StartsWith("++")) pageName = pageName.Substring(2);
					...
					string currentNamespace = currentPageFullName != null ? NameTools.GetNamespace(currentPageFullName) : null;
				...
				PageContent transcludedPage = Pages.FindPage(wiki, pageName);

				// Avoid circular transclusion
				bool transclusionAllowed =
					transcludedPage != null &&
						(currentPageFullName != null &&
						 transcludedPage.FullName != currentPageFullName ||
				...
				if(transclusionAllowed) {
					...
					bool canView = authChecker.CheckActionForPage(transcludedPage.FullName, Actions.ForPages.ReadPage, currentUsername, currentGroups);
					if(canView) {
						dummy = new StringBuilder();
						dummy.Append(@"<div class=""transcludedpage"">");
						dummy.Append(FormattingPipeline.FormatWithPhase3(
							wiki,
							FormattingPipeline.FormatWithPhase1And2(wiki, transcludedPage.Content,
							false, FormattingContext.TranscludedPageContent, transcludedPage.FullName),
							FormattingContext.TranscludedPageContent, transcludedPage.FullName));
						dummy.Append("</div>");
						sb.Insert(match.Index, dummy.ToString());
					}
					else {
   ...
   //         if (ns == null) ns = "";
			...
			foreach(string nspace in allowedNamespaces) {
				sb.AppendFormat("<option{0} value=\"{1}\">{2}</option>", nspace == ns ? " selected=\"selected\"" : "",
                    nspace, nspace == "" ? WebUtility.HtmlEncode(Settings.GetRootNamespaceName(wiki)) : nspace);
			}
