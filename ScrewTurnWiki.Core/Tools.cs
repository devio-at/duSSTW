...
namespace ScrewTurn.Wiki {
	...
	public static class Tools {
        ...
        public static string GetIncludes(UrlHelper Url, string wiki, string nspace) {
				...
				foreach(string cssFile in cssList) {
					if(Path.GetFileName(cssFile).IndexOf("_") != -1) {
						...
						if(firstChunk.Equals("screen") || firstChunk.Equals("print") || firstChunk.Equals("all") ||
							...
							result.Append(@"<link rel=""stylesheet"" media=""" + firstChunk + @""" href=""" + Url.Content(cssFile) + @""" type=""text/css"" />" + "\n");
						}
						else {
							result.Append(@"<link rel=""stylesheet"" href=""" + Url.Content(cssFile) + @""" type=""text/css"" />" + "\n");
						}
					}
					else {
						result.Append(@"<link rel=""stylesheet"" href=""" + Url.Content(cssFile) + @""" type=""text/css"" />" + "\n");
			...
			if (customEditorCss!= null && customEditorCss.Count>0) result.AppendFormat(@"<link rel=""stylesheet"" href=""{0}"" type=""text/css"" />" + "\n", Url.Content(customEditorCss[0]));
			else result.Append(@"<link rel=""stylesheet"" href=""" + Url.Content("~/Content/Themes/Editor.css") + @""" type=""text/css"" />" + "\n");
			// OpenSearch
            result.AppendFormat(@"<link rel=""search"" href=""" + Url.Content("~/Search/Opensearch") + @""" type=""application/opensearchdescription+xml"" title=""{0}"" />" + "\n", Settings.GetWikiTitle(wiki) + " - Search");

            result.Append(@"<link rel=""stylesheet"" href=""" + Url.Content("~/Content/Themes/prettyPhoto.css") + @""" type=""text/css"" />" + "\n"); // TODO: Move to adjust styles

			List<string> jsFiles = Themes.ListThemeFiles(wiki, nameTheme, ".js");
			if(jsFiles != null) {
				foreach(string jsFile in jsFiles) {
					result.Append(@"<script src=""" + Url.Content(jsFile) + @""" type=""text/javascript""></script>" + "\n");
				}
			}
			...
			if(iconsList != null) {
				...
				if(icons.Length > 0) {
					result.Append(@"<link rel=""shortcut icon"" href=""" + Url.Content(icons[0]) + @""" type=""");
		...
		public static string GetJavaScriptIncludes(UrlHelper Url) {
			StringBuilder buffer = new StringBuilder(100);

			foreach(string js in Directory.GetFiles(GlobalSettings.JsDirectory, "*.js")) {
				buffer.Append(@"<script type=""text/javascript"" src=""" + Url.Content("~/") + GlobalSettings.JsDirectoryName + "/" + Path.GetFileName(js) + @"""></script>" + "\n");
			}
		...
		public static string DetectCurrentWiki() {
            ...
            if (HttpContext.Current?.Request?.Url?.Host != null)
            {
                try
                {
